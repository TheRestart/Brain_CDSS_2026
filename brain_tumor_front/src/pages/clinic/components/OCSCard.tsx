...
export default function OCSCard({
  patientId,
  ocsList,
  onOCSCreated: _onOCSCreated,
}: OCSCardProps) {
  ...
  // OCS 상세 보기
  const handleViewOCS = (ocs: OCSListItem) => {
    if (ocs.job_role === 'RIS') {
    ...
    <div className="clinic-card">
      <div className="clinic-card-header">
        <h3>
          ...
          OCS
          <span className="ocs-counts">
            <span className="pending-count">{statusCounts.pending}</span>
            /
            <span className="completed-count">{statusCounts.completed}</span>
          </span>
        </h3>
        <button className="btn btn-sm btn-primary" onClick={handleNewOCS}>
          + 새 OCS
        </button>
      </div>

      {/* 필터 탭 */}
      <div className="ocs-filter-tabs">
        <button
      ...
      <div className="clinic-card-body ocs-list-body">
        {filteredList.length === 0 ? (
          <div className="empty-state">
            <div className="empty-state-icon">📝</div>
            <div className="empty-state-text">등록된 OCS가 없습니다.</div>
          </div>
        ) : (
          <div className="ocs-list">
            {filteredList.slice(0, 10).map((ocs) => (
              <div
                key={ocs.id}
                className="list-item ocs-item"
                onClick={() => handleViewOCS(ocs)}
              >
...
// 하위 호환성을 위한 별칭 (OrderCard로도 import 가능)
export { OCSCard as OrderCard };
